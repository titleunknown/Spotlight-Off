...
/// A currently-mounted volume that has Spotlight indexing turned off.
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @objc func openSettings() {
        ...
        driveMonitor.refreshDisabledVolumes()
...
class DriveMonitor: ObservableObject {
    ...
    @Published var disabledVolumes: [MountedVolume] = []
    @Published var isScanningVolumes = false
    ...
    // MARK: - Re-enable Support

    /// Scans every mounted, local, browsable volume and publishes the ones whose
    /// Spotlight indexing is currently disabled. Runs blocking `mdutil` probes off
    /// the main thread, so it dispatches back to main to update the published list.
    func refreshDisabledVolumes() {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            DispatchQueue.main.async {
                self.disabledVolumes = sorted
                self.isScanningVolumes = false
            }
        }
    }

    /// Turns Spotlight indexing back on for a volume, then rescans so the list
    /// reflects the new state. Also drops any matching "processed" history entry.
    func reEnableIndexing(_ volume: MountedVolume) {
        LogStore.shared.log("\"\(volume.name)\" — re-enabling Spotlight…", kind: .info)
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            guard let self else { return }
            guard FileManager.default.fileExists(atPath: volume.path) else {
                LogStore.shared.log("\"\(volume.name)\" — volume no longer mounted, skipped", kind: .info)
                self.refreshDisabledVolumes()
                return
            }
            let (ok, detail) = self.runMdutil(path: volume.path, enable: true)
            if ok {
                LogStore.shared.log("\"\(volume.name)\" — Spotlight re-enabled ✓", kind: .success)
                DispatchQueue.main.async {
                    self.history.removeAll { $0.path == volume.path }
                    self.saveHistory()
                    self.onHistoryChanged?()
                }
            } else {
                LogStore.shared.log("\"\(volume.name)\" — failed to re-enable: \(detail)", kind: .failure)
            }
            self.refreshDisabledVolumes()
        }
    }
    ...
    /// Runs `mdutil -i off` (or `-i on` when `enable` is true) and returns
    /// (success, detail string for failures).
    private func runMdutil(path: String, enable: Bool = false) -> (Bool, String) {
        let p = Process()
        p.executableURL = URL(fileURLWithPath: "/usr/bin/mdutil")
        p.arguments = ["-i", enable ? "on" : "off", path]
        let outPipe = Pipe()
        let errPipe = Pipe()
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                // ── Re-enable Indexing Section ───────────────────────────────
                HStack {
                    SectionHeader("Re-enable Indexing")
                    Spacer()
                    Button {
                        monitor.refreshDisabledVolumes()
                    } label: {
                    ...
                    .help("Rescan mounted volumes")
                    .padding(.top, 20).padding(.trailing, 20)
                }

                if monitor.disabledVolumes.isEmpty {
                    VStack(spacing: 8) {
                        if monitor.isScanningVolumes {
                            ...
                            Text("No volumes with indexing disabled")
                                .foregroundColor(.secondary).font(.subheadline)
                            Text("Mounted volumes that currently have Spotlight indexing turned off will appear here.")
                    ...
                    VStack(spacing: 0) {
                        ForEach(Array(monitor.disabledVolumes.enumerated()), id: \.element.id) { index, volume in
                            if index > 0 { Divider().padding(.leading, 44) }
                            VolumeRow(volume: volume) {
                                monitor.reEnableIndexing(volume)
                            }
                        }
                    }
                    .background(Color(NSColor.controlBackgroundColor))
                    ...
                    Text("Re-enable turns Spotlight indexing back on for that volume.")
        ...
        .onAppear {
            if monitor.disabledVolumes.isEmpty { monitor.refreshDisabledVolumes() }
        }
    }
}
    ...
    var body: some View {
        HStack(spacing: 12) {
            ZStack {
                ...
                Image(systemName: "externaldrive.badge.xmark").foregroundColor(.orange)
                    .font(.system(size: 14))
            }
            VStack(alignment: .leading, spacing: 2) {
                Text(volume.name).fontWeight(.medium).font(.system(size: 13))
                Text(volume.path).font(.caption).foregroundColor(.secondary)
                    .lineLimit(1).truncationMode(.middle)
            }
            Spacer()
            Button("Re-enable", action: onEnable)
                .buttonStyle(.bordered).controlSize(.small)
        }
